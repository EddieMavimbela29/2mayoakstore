...
export default function AdminJobEditScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { data } = await axios.get(`/api/admin/jobs/${productId}`);
  ...
  const submitHandler = async ({
    ...
    try {
      ...
      await axios.put(`/api/admin/jobs/${productId}`, {
        name,
        slug,
        price,
        category,
  ...
  return (
    <Layout title={`Edit Job ${productId}`}>
        ...
        <div className="md:col-span-3">
          {loading ? (
            ...
            >
              ...
              <div className="mb-4">
                <label htmlFor="price">faculty</label>
                <input
                  type="text"
                  className="w-full"
                  id="faculty"
...
AdminJobEditScreen.auth = { adminOnly: true };
